...
public record SemesterPlan(UniversitySubject[] subjects, SubjectRequirement[] subjectRequirements,
    ...
    public SemesterPlan {
        if (subjects == null) {
            throw new IllegalArgumentException("Cannon create semester plan with null subjects array");
        }
        if (subjectRequirements == null) {
            throw new IllegalArgumentException("Cannon create semester plan with null subjects requirements array");
        }
        if (minimalAmountOfCredits < 0) {
            throw new IllegalArgumentException("minimalAmountOfCredits cannot be negative");
        }
    }
}
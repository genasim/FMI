...
void Vector<T>::eraseAll(const T &element) noexcept {
    size_t deleted = std::addressof(_data[0]) == std::addressof(element);

    for (size_t i = 1; i < _size; i++) {
        if (_data[i] == std::addressof(element)) {
            ++deleted;
            continue;
        }
...
void Vector<T>::erase(const T &element) noexcept {
    bool found = std::addressof(_data[0]) == std::addressof(element);
    for (size_t i = 1; i < _size; i++) {
        if (!found && std::addressof(_data[i]) == std::addressof(element)) {
            found = true;
            continue;
        }
